...
unsigned test_compare_chain(int verbose)
{
	...
	err = 0;
	result = ehbi_greater_than(&bi1, &bi1);
	if (err) {
		++failures;
		STDERR_FILE_LINE_FUNC(log);
		log->append_s(log, "error ");
	...
	err = 0;
	result = ehbi_greater_than(&bi1, &bi3);
	if (err) {
		++failures;
		STDERR_FILE_LINE_FUNC(log);
		log->append_s(log, "error ");
	...
	err = 0;
	result = ehbi_greater_than(&bi3, &bi1);
	if (err) {
		++failures;
		STDERR_FILE_LINE_FUNC(log);
		log->append_s(log, "error ");
